...
impl Relay {
    ...
    pub async fn deliver_queued(&self, pool: &PgPool, team_id: Uuid, email: &str, tx: &Tx) {
        let rows: Vec<(Uuid, String, String, String)> = sqlx::query_as(
            "DELETE FROM envelope_queue WHERE team_id = $1 AND recipient_id = $2 AND expires_at > NOW() RETURNING id, sender_id, encrypted_payload, EXTRACT(EPOCH FROM created_at)::text",
        )
        .bind(team_id)
        .bind(email)
        .fetch_all(pool)
        ...
        for (id, sender, payload, ts) in rows {
